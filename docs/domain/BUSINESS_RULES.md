...
12. A platform admin may inactivate an active ASI membership through an audited admin action. Inactivation must remove protected product access by setting the user-level membership/subscription gate out of the active states, without blocking the user account itself.
13. Tenant plans unlock workspace capacity and features, but they do not activate individual user access by default.
14. Roles decide what a user can do; plans decide what capacity/features are enabled; both checks are required.
15. Tenant kind restrictions apply even when a tenant has a paid plan.
16. The canonical commercial plan model lives in `docs/product/COMMERCIAL_PLAN_MODEL.md`.
...
14. User-owned authenticated workflows must live under `/account/*`; employer tenant workflows must live under `/workspace/*`. Historical `/candidate/*` routes may exist only as redirects to the matching `/account/*` destination.
...
17. Authenticated users with incomplete base profile setup must be redirected to `/account/profile` before workspace, admin, or other account surfaces. The guided first-run setup is a profile state, not a standalone module. Required base data is useful full name, useful display name, supported locale, and two-letter country code; avatar, CV, candidate depth, and company/operator requests are optional follow-up work.
