...
Do not make the talent-pool insert policy read recruiter-visible candidate profiles through invoker-context RLS. The policy must preserve `candidate_directory:read`, bind `saved_by_user_id` to `auth.uid()`, and evaluate profile visibility through a narrow boolean-only security-definer predicate. The candidate detail side sheet must keep every footer action inside its narrow panel; long actions must occupy their own row instead of relying on viewport breakpoints that can force horizontal overflow.
