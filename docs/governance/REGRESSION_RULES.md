...
Do not upload or replace raster profile photos, company logos, or operator-request logos immediately after file selection. PNG, JPG, and WEBP identity images must first open a crop/positioning dialog so the user can choose the visible framing before the saved file is prepared. SVG logos stay as direct vector uploads. The framing interaction should use direct manipulation, such as drag to position and pinch or wheel to zoom in or out, rather than separate horizontal/vertical positioning controls.
...
Do not manage platform administrators through ad hoc table edits, client-only checks, or broad `platform_admin` access. The post-bootstrap path for creating custom platform roles, editing the permission set assigned to any platform role, assigning or revoking `platform_roles`, deleting custom roles, and inspecting RBAC/audit reports must stay inside `/admin/access-control`, require active `platform_owner` authority server-side, emit semantic audit events, keep locked system role metadata immutable, and prevent revoking the last active `platform_owner`.
...
Do not ship campaign emails as generic stacked templates with default typography, repetitive cards, or decorative choices disconnected from the promoted event. Event invitations must derive their typography, palette, imagery, spacing, and editorial hierarchy from the campaign identity; keep the copy concise; make dates and locations actionable; use purposeful iconography only where it improves scanning; and preserve a polished responsive layout in clients that do not support web fonts or hover states. Detail icons must use simple, universally recognizable pictograms supplied as inline CID PNGs with meaningful alternative text; do not substitute cryptic font glyphs, emoji, or text characters for interface icons. Rounded icon tiles and detail cards must not retain square table outlines around their backgrounds. Do not insert standalone ornamental separators, motion blocks, or visual effects that add height without adding meaning. Campaign visuals, fonts, icons, posters, and QR codes must not depend on public asset hosting: use email-safe system fonts, self-contained markup, and MIME/CID inline attachments so the delivered message renders without loading visual resources from the app. External URLs are reserved for intentional recipient actions such as registration, calendar, maps, and support, and every such destination must be verified before handoff.

### R-133 — Pending work is tracked as a checklist on TASK-255, not as new Linear issues
Do not open new Linear issues for follow-up work. The workspace is on the free plan and rejects issue creation, so every follow-up must be recorded as an unchecked checklist item in the `TASK-255` comment for its area (`Calidad, CI y observabilidad`, `Frontend y PWA`, `Mantenibilidad e higiene`, `Rendimiento de aplicacion`), editing that existing comment rather than adding a new one, and checked off there —with the commit that closed it— once the work lands. This applies to every kind of pending work: implementation, documentation, deployment, manual verification, external configuration, and unexplained test failures.
...
### R-135 — Admin membership and user queues must stay newest-first and date-visible
Do not prioritize role ownership or another secondary attribute above creation time in administrative membership or platform-user lists. The newest record must appear first, each card must show its creation date, and the compact hierarchy must keep identity, state, operational metadata, and available actions easy to distinguish on mobile and desktop. Membership records must also keep the three-step `Solicitud`, `Pago`, and `Acceso` path visible so the administrator can understand what remains before activation.
...
Do not let `/legal` or its policy documents return to oversized marketing headings, tall mastheads, heavily padded cards, wide vertical gaps between clauses, visible document version numbers, effective-date badges, or explanatory copy about each policy having a citable URL. The legal center must keep a shallow masthead, a heading below display scale that stays on one line at desktop widths, compact low-padding policy cards, and a brief security notice; do not enlarge these surfaces back into feature-style panels or expose versions in cards, metadata, actions, or change-history rows. Export/print and change-history controls must remain hidden unless the session has active `platform_owner` authority; generic admin-console access is insufficient. Every policy route must preserve restrained titles, tight metadata and tabs, and readable low-density document sections across mobile and desktop.
