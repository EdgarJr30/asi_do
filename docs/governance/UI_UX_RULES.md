...
12. Administration screens should stay consolidated by operational domain: Aprobaciones includes territorial authority invitations, Finanzas includes payments plus donations, and Comunicaciones includes email pipeline plus notifications. Prefer compact statbars, tabs, list rows, and collapsible metadata over large KPI cards, duplicated settings modules, or raw JSON blocks. Dense admin modules should use the full shell width within the standard shell padding instead of a second centered max-width container.
