...
8. Candidate application history screens must favor a compact searchable list over repeated large cards. Use summary chips, segmented status filters, a single list surface with separated rows, clear empty states, and mobile rows that keep the job, status, date, and job-detail action visible without horizontal scrolling.
