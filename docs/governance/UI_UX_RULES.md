...
13. **The institutional footer must stay compact.** Preserve its legal, payment, navigation, and environment information, but use a dense two-column link grid, restrained card padding, and short vertical spacing so it does not read as another full landing section. Footer links must retain a minimum 44px touch target.
