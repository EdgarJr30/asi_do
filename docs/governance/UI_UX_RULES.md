...
13. **The institutional footer must stay compact.** Preserve its legal, payment, navigation, and environment information, but use restrained card padding, small payment marks, and short vertical spacing so it does not read as another full landing section. Navigation links must stack in one column within each group, remain on one line, and use a moderately compact visual badge inside a minimum 44px touch target.
...
7. Candidate job detail and application flows must keep the job context visible without turning the page into stacked heavy cards. Job details should use clear content sections plus a sticky action rail on larger viewports, while application forms should use short guided steps with inline validation, local draft persistence, and clear already-applied states instead of one long wall of fields or duplicate primary CTAs.
8. Candidate application history screens must favor a compact searchable list over repeated large cards. Use the summary stat chips as the status filters, a single list surface with separated rows, clear empty states, and mobile rows that keep the job, status, date, and job-detail action visible without horizontal scrolling. Do not add a second segmented status control that duplicates the stat-chip filters.
9. Membership status screens must avoid duplicate progress/status summaries. Use one compact membership card for category, status, dates, and remaining term; segmented tabs for summary, route, and receipts; thin definition/timeline rows instead of repeated large cards; and a right rail for renewal/support that stacks below the main content on mobile. Receipt histories must render collapsed by default and paginate through the shared `Pagination` primitive when more than four receipts exist.
10. Candidate profile editing must keep the normal profile surface organized as a compact tabbed editor with a stat strip and a summary/action rail that stacks below on mobile. Do not replace it with one long form or nested card-heavy layout; keep repeatable sections labeled, scannable, and touch-friendly.
11. Workspace activity history must use a compact stat strip, segmented filters with counts, grouped timeline rows, and lightweight hover elevation instead of repeated large cards. Keep the shared workspace shell intact, paginate dense activity feeds, and make the layout wrap cleanly on mobile without hiding core filter controls.
12. Workspace reports must stay lightweight, wide, and scan-first: use the full available shell content width, a compact period selector, small KPI tiles, unframed funnel/conversion sections, animated horizontal bars, and clear empty/loading/error states. Do not turn report summaries into a card-heavy dashboard, constrain them with narrow handoff containers, or rebuild shared workspace shell chrome inside feature pages.
13. Long legal documents must keep their table of contents sticky beneath the public navigation on desktop so readers retain section context while scrolling; keep the document content single-column on smaller viewports.
...
6. Customer-facing media-led hero sections may break out to full viewport width when the image or experience is the primary content, while keeping the rest of the page on the shared container rhythm. Full-bleed image heroes should match the active asset aspect ratio at each breakpoint to avoid unintended letterboxing or cropping.
7. Public mission/project pages must keep funding information scannable: lead with the mission message and primary donation/application actions, use restrained media, show a focused current-project selection with images and funding amounts, and group older projects by year instead of loading dense archive content into the main page.
...
- pricing section may remain implemented but hidden; if hidden, public navigation and CTAs must not point to it, and pricing CTAs that stay visible must route to the institutional membership dues surface at `/membership/categories`
- final CTA region
...
8. If registration intake is closed, public CTAs that would create a platform account or submit a membership application must render as disabled or clearly closed actions while preserving read-only product exploration.
9. Visible opportunity workflow copy must use ASI-native labels: `Responsable de oportunidad` for accountable owners, `Coordinador de oportunidad` for users who create/manage opportunities or move applicants, and `Revisor de aplicaciones` for users who review, rate, comment, or optionally move applicants.
10. Avoid visible `recruiter` and `hiring manager` labels in navigation, buttons, empty states, helper text, notifications, and product copy. Legacy technical identifiers may appear only in developer-facing diagnostics or explicit permission-code references.
...
10. The ASI brand mark and lockup in authenticated platform sidebars must always navigate to the shared account `Inicio`, regardless of whether the user is currently in candidate, workspace, or administration modules. The public platform shell keeps its own public home as the brand destination. Keep this brand action visually stable on pointer hover; use the keyboard focus ring for accessibility without adding a hover fill or animated hover treatment.
...
6. Top-level workspace navigation should expose distinct jobs-to-be-done. If a page is mostly an expanded detail of the `Resumen` dashboard, link to it from the relevant dashboard panel instead of promoting it as a separate persistent module.
...
5. The `/account/jobs` jobs board should use compact selectable rows plus a focused detail pane on desktop; avoid oversized repeated cards, and after filters or sort changes leave the detail pane unselected until the user chooses a vacancy.
...
8. The workspace `Proceso de selección` board must keep the shared app shell intact, render as a compact Kanban with page-level height bounded to the viewport, and place vertical overflow inside each stage column. Columns must stay wide enough for candidate names to scan comfortably, and long stage lists should reveal candidate cards progressively with the official ASI loader near the column bottom instead of mounting every card at once. Horizontal wheel or trackpad scrolling must keep moving the board even when the pointer is over a stage column. Candidate cards should stay row-sized and avoid repeating status pills that are already communicated by the column.
