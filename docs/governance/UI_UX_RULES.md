...
9. Membership status screens must avoid duplicate progress/status summaries. Use one compact membership card for category, status, dates, and remaining term; segmented tabs for summary, route, and receipts; thin definition/timeline rows instead of repeated large cards; and a right rail for renewal/support that stacks below the main content on mobile.
