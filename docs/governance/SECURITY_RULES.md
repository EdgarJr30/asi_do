...
32. Successful authentication sessions must be copied from `auth.sessions` into a dedicated access-log store protected by RLS and revoked direct grants. Administrative reads require `audit_log:read` or platform-admin authority server-side and must themselves be audited.
