...
`RESEND_WEBHOOK_SECRET` es exclusivo de cada endpoint/entorno. La Edge Function `resend-webhook` valida el cuerpo crudo y los headers `svix-*`; nunca debe reutilizarse como llave de envío ni exponerse al frontend. Para desarrollo local se aceptan los alias `RESEND_API_KEY_DEV` y `EMAIL_FROM_ADDRESS_DEV`; al desplegar se copian a los nombres canónicos sin sufijo.
