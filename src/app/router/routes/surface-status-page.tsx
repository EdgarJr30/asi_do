...
const SUPPORT_EMAIL = 'hola@asidominicana.do';
    ...
    switch (surface) {
      case 'candidate':
        return {
          ...
          description:
            'Tu sesión sigue activa, pero esta sección no está disponible para tu acceso actual.',
      ...
      case 'workspace':
        return {
          ...
          description:
            'Tu sesión pertenece al workspace, pero este módulo requiere un permiso adicional.',
      ...
      case 'admin':
        return {
          ...
          description:
            'La consola de plataforma reconoce tu sesión, pero no tienes el permiso necesario para esta sección.',
      ...
      case 'auth':
        return {
          ...
          description:
            'Vuelve al acceso principal para continuar desde una ruta válida.',
      ...
      case 'institutional':
        return {
          ...
          description:
            'Vuelve al portal principal de ASI para continuar desde una ruta válida.',
  ...
  switch (surface) {
    case 'institutional':
      return {
        ...
        description:
          'El portal institucional sigue disponible, pero esta ruta no forma parte de la navegación principal.',
    ...
    case 'candidate':
      return {
        ...
        description:
          'La app de candidato sigue disponible, pero esta ruta ya no existe o no forma parte del flujo actual.',
        actionLabel: 'Ir a mi perfil',
        actionHref: surfacePaths.candidate.profile,
      };
    case 'workspace':
      return {
        ...
        description:
          'El workspace sigue activo, pero esta ruta no pertenece a la experiencia canónica de empresa.',
        actionLabel: 'Volver al workspace',
        actionHref: surfacePaths.workspace.root,
      };
    case 'admin':
      return {
        ...
        description:
          'La consola admin sigue disponible, pero esta ruta no pertenece a la navegación operativa actual.',
        actionLabel: 'Volver al admin',
        actionHref: surfacePaths.admin.root,
      };
    case 'auth':
      return {
        ...
        description:
          'Vuelve al flujo principal de autenticación para continuar desde una ruta válida.',
        actionLabel: 'Ir a sign in',
        actionHref: surfacePaths.auth.signIn,
      };
    default:
      return {
        ...
        description:
          'La landing comercial y las oportunidades para miembros siguen disponibles dentro de la plataforma, pero esta ruta no forma parte de esa experiencia.',
        actionLabel: 'Volver a la plataforma',
        actionHref: surfacePaths.storefront.home,
      };
  }
...
export function SurfaceStatusPage({
  ...
  function handleSearch(event: FormEvent) {
    ...
    void navigate(
      trimmed
        ? `${surfacePaths.storefront.jobs}?q=${encodeURIComponent(trimmed)}`
    ...
    >
        ...
        <motion.div variants={cardReveal} className="space-y-5">
          ...
          <div className="space-y-3">
            <h1 className="text-[2rem] font-bold leading-tight tracking-[-0.03em] text-(--app-text) sm:text-[2.6rem]">
              {isNotFound
                ? 'Ups, esta página no está disponible'
                : content.title}
            </h1>
          ...
          {isNotFound ? (
            <form onSubmit={handleSearch} className="space-y-2">
              <label
                ...
                className="text-[0.82rem] font-medium text-(--app-text-muted)"
              >
                ¿Qué estás buscando?
              </label>
              <div className="flex items-center gap-2.5 rounded-card border border-(--app-border) bg-(--app-surface) px-3.5 focus-within:border-primary-400 focus-within:ring-2 focus-within:ring-(--app-ring)">
                <Search
                  aria-hidden="true"
                  className="size-4 text-(--app-text-subtle)"
                />
                <input
                  id={searchFieldId}
                  value={searchTerm}
          ...
          return (
            ...
            >
              ...
              >
                ...
                <p className="flex-1 text-xs leading-5 text-(--app-text-muted)">
                  {link.description}
                </p>
      ...
      >
        ...
        <a
          ...
          className="font-semibold text-primary-600 hover:text-primary-700 dark:text-primary-300"
        >
          {SUPPORT_EMAIL}
        </a>{' '}
        o visita nuestra{' '}
